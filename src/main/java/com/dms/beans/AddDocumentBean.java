...
public class AddDocumentBean implements Serializable {
    ...
    public void documentClassChanged() {
        try {
            ...
            UIUtils.generatePropertiesInputs(selectedDocumentClass.getPropertiesList(), propertiesPanelGrid, localeBean.getLocale(), Constants.OPERATION_ADD_DOCUMENT, currentUserBean.getUser());
        } catch (Exception e) {
            PrimeFaces.current()
                .ajax()
    ...
    public void createNewDocument() {
        try {
            ...
            Map<String, Object> propertiesMap = UIUtils.getPropertiesInputsValues(propertiesPanelGrid, selectedDocumentClass.getSymbolicName(), null);
            ...
            setCustomProperties();
            utilsRepository.insertDocument(selectedDocumentClass.getTableName()
                .getValue(), selectedDocumentClass.getPropertiesList());
            ...
            PrimeFaces.current()
                .ajax()
                .addCallbackParam("successDialog", true);
        } catch (Exception e) {
            PrimeFaces.current()
                .ajax()
                .addCallbackParam("errorDialog", true);
    ...
    private void setCustomProperties() {
        selectedDocumentClass.getPropertiesList()
            .add(new Property(CustomColumnsEnum.CONTENT_SIZE.getValue(), 0));
        selectedDocumentClass.getPropertiesList()
            .add(new Property(CustomColumnsEnum.DOCUMENT_CLASS_ID.getValue(), selectedDocumentClass.getId()));
        selectedDocumentClass.getPropertiesList()
            .add(new Property(CustomColumnsEnum.CREATED_BY_ID.getValue(), currentUserBean.getUser()
                .getId()));
        selectedDocumentClass.getPropertiesList()
    ...
    private void removeCustomProperties() {
        for (Iterator<Property> iterator = selectedDocumentClass.getPropertiesList()
            .iterator(); iterator.hasNext();) {
            Property property = iterator.next();
            if (property.getColumnName()
                .equalsIgnoreCase(CustomColumnsEnum.CONTENT_SIZE.getValue())
                || property.getColumnName()
                    .equalsIgnoreCase(CustomColumnsEnum.DOCUMENT_CLASS_ID.getValue())
                || property.getColumnName()
                    .equalsIgnoreCase(CustomColumnsEnum.CREATED_BY_ID.getValue())
                || property.getColumnName()
                    .equalsIgnoreCase(CustomColumnsEnum.DATE_CREATED.getValue())
                || property.getColumnName()
                    .equalsIgnoreCase(CustomColumnsEnum.FILE_NAME.getValue())
                || property.getColumnName()
                    .equalsIgnoreCase(CustomColumnsEnum.FULL_PATH.getValue())
                || property.getColumnName()
                    .equalsIgnoreCase(CustomColumnsEnum.MIME_TYPE.getValue())
                || property.getColumnName()
                    .equalsIgnoreCase(CustomColumnsEnum.ORIGINAL_FILE_NAME.getValue())) {
                iterator.remove();
            }
        }
    }
